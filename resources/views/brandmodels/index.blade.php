...
<div class="space-y-8">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-3xl font-bold text-slate-800">👥 Gestión de Modelos</h1>
            <p class="text-slate-500">Administra los modelos de los vehículos.</p>
        </div>
        ...
        <table class="min-w-full divide-y divide-slate-200 text-sm">
            <thead class="bg-emerald-50">
                <tr>
                    ...
                    <th class="px-4 py-3 text-left font-semibold text-slate-600 uppercase">Código</th>
                    <th class="px-4 py-3 text-left font-semibold text-slate-600 uppercase">Descripción</th>
                    <th class="px-4 py-3 text-left font-semibold text-slate-600 uppercase">Marca</th>
            ...
            <tbody class="divide-y divide-slate-100">
                @forelse($models as $m)
                    <tr class="hover:bg-emerald-50/40 transition">
                        <td class="px-4 py-3 text-slate-600">{{ $m->id }}</td>
                        <td class="px-4 py-3 font-mono text-slate-700">{{ $m->name }}</td>
                        <td class="px-4 py-3 text-slate-700 font-medium">{{ $m->code }}</td>
                        <td class="px-4 py-3 text-slate-700">{{ $m->description }}</td>
                        <td class="px-4 py-3 text-slate-500">{{ $m->brand->name }}</td>
                        <td class="px-4 py-3 flex justify-center gap-2">
                            <a href="{{ route('brand-models.edit', $m->id) }}"
                               data-turbo-frame="modal-frame"
                            ...
                            <button type="button"
                                    ...
                                    data-id="{{ $m->id }}"
                                    data-url="{{ route('brand-models.destroy', $m->id) }}"
                                    title="Eliminar">
