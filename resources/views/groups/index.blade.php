@extends('layouts.app')
@section('title', 'Gestión de Grupo — RSU Reciclaje')
...
<div class="space-y-8">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-3xl font-bold text-slate-800">👥 Gestión de Grupos</h1>
            <p class="text-slate-500">Administra los grupos de personal.</p>
        </div>

        <a href="{{ route('groups.create') }}"
           data-turbo-frame="modal-frame"
           class="flex items-center gap-2 px-4 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 transition">
           <i class="fa-solid fa-user-plus"></i> Nuevo Grupo
        </a>
    </div>
        ...
        <table class="min-w-full divide-y divide-slate-200 text-sm">
            <thead class="bg-emerald-50">
                <tr>
                    ...
                    <th class="px-4 py-3 text-left font-semibold text-slate-600 uppercase">Turno</th>
            ...
            <tbody class="divide-y divide-slate-100">
                @forelse($groups as $g)
                    <tr class="hover:bg-emerald-50/40 transition">
                        <td class="px-4 py-3 text-slate-600">{{ $g->id }}</td>
                        <td class="px-4 py-3 font-mono text-slate-700">{{ $g->name }}</td>
                        <td class="px-4 py-3 text-slate-700 font-medium">{{ $g->zone->name }}</td>
                        <td class="px-4 py-3 text-slate-700">{{ $g->schedule->name }}</td>
                        <td class="px-4 py-3 text-slate-500">{{ $g->vehicle->name }}</td>
                        <td class="px-4 py-3 flex justify-center gap-2">
                            ...
                            <button type="button"
                                    ...
                                    data-id="{{ $g->id }}"
                                    data-url="{{ route('groups.destroy', $g->id) }}"
                                    title="Eliminar">
