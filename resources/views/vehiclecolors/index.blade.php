@extends('layouts.app')
@section('title', 'Gestión de Colores — RSU Reciclaje')
...
<div class="space-y-8">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-3xl font-bold text-slate-800">🎨 Gestión de Colores</h1>
            <p class="text-slate-500">Administra los colores de los vehículos.</p>
        </div>

        <a href="{{ route('vehiclecolors.create') }}"
        ...
        <table class="min-w-full divide-y divide-slate-200 text-sm">
            <thead class="bg-emerald-50">
                <tr>
                    ...
                    <th class="px-4 py-3 text-left font-semibold text-slate-600 uppercase">Color</th>
                    <th class="px-4 py-3 text-left font-semibold text-slate-600 uppercase">Nombre</th>
                    <th class="px-4 py-3 text-left font-semibold text-slate-600 uppercase">Código RGB</th>
            ...
            <tbody class="divide-y divide-slate-100">
                @forelse($colors as $c)
                    <tr class="hover:bg-emerald-50/40 transition">
                        <td class="px-4 py-3 text-slate-600">{{ $c->id }}</td>
                        <td class="px-4 py-3 font-mono text-slate-700">
                            <div class="h-8 w-20 rounded-md border" style="background: {{ $c->rgb_code }}"></div>
                        </td>
                        <td class="px-4 py-3 font-mono text-slate-700">{{ $c->name }}</td>
                        <td class="px-4 py-3 text-slate-700">{{ $c->rgb_code }}</td>
                        <td class="px-4 py-3 flex justify-center gap-2">
                            <a href="{{ route('vehiclecolors.edit', $c->id) }}"
                               data-turbo-frame="modal-frame"
                            ...
                            <button type="button"
                                    ...
                                    data-id="{{ $c->id }}"
                                    data-url="{{ route('vehiclecolors.destroy', $c->id) }}"
                                    title="Eliminar">
                                <i class="fa-solid fa-trash"></i>
                            </button>

                            <a href="{{ route('vehiclecolors.show', $c->id) }}" class="px-2 py-1 bg-slate-100 text-slate-700 rounded-md hover:bg-slate-200" title="Ver">
