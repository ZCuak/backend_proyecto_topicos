    ...
    <div id="flyonui-modal-container">
        ...
        <div id="editModal"
            ...
            <div class="flyonui-dialog relative bg-white rounded-2xl shadow-2xl w-full 
                        max-w-3xl sm:mx-4 overflow-hidden 
                ...
                <div class="flyonui-header flex justify-between items-center px-6 py-4 border-b border-slate-200 bg-slate-50">
                    <h3 class="font-bold text-lg text-slate-800 flex items-center gap-2">
                        <i class="fa-solid fa-pen text-yellow-600"></i> Editar color
                    </h3>
                    <button type="button"
                            onclick="FlyonUI.modal.close('editModal')"
                ...
                <div class="flyonui-body p-6 bg-white">
                    <form action="{{ route('vehiclecolors.update', $color->id) }}"
                          method="POST"
                          data-turbo-frame="modal-frame"
                          class="space-y-6">
                        @csrf
                        @method('PUT')
                        @include('vehiclecolors._form', ['buttonText' => 'Actualizar'])
                    </form>
                </div>
            </div>
        </div>
